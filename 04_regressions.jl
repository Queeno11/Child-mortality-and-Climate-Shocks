...
file = CSV.File(
    "D:\\World Bank\\Paper - Child mortality and Climate Shocks\\Data\\Data_out\\DHSBirthsGlobal&ClimateShocks_v9.csv"; limit=10
)

for m in ["1", "3", "6", "9", "12", "48"]
    ...
    columns_to_include = vcat(columns_to_include, controls2, [:chb_month, :chb_year])
    ...
    # Urban & Rural
    CustomModels.run_heterogeneity_dummy(df, controls1, 1, "rural", "", "", [m])

    # Mechanisms

    CustomModels.run_heterogeneity_dummy(df, controls1, 1, "pipedw", "", "", [m])
    CustomModels.run_heterogeneity_dummy(df, controls1, 1, "href", "", "", [m])
    CustomModels.run_heterogeneity_dummy(df, controls1, 1, "hhelectemp", "", "", [m])

    # Gender
    CustomModels.run_heterogeneity_dummy(df, controls1, 1, "child_fem", "", "", [m])
    ...
    for climate_band in climate_bands
        ...
        for controls in [controls1]
            try
                ...
                CustomModels.run_models(df_climate_bands, controls, "climate_bands\\", " - $(climate_band)", [m])
            catch
